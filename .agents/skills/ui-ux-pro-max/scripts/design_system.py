...
def format_ascii_box(design_system: dict) -> str:
    ...
    lines.append("+" + "-" * w + "+")
    lines.append("|" + format_line(f"  TARGET: {project} - RECOMMENDED DESIGN SYSTEM") + "|")
    lines.append("+" + "-" * w + "+")
    lines.append("|" + " " * w + "|")

    # Pattern section
    lines.append("|" + format_line(f"  PATTERN: {pattern.get('name', '')}") + "|")
    if pattern.get('conversion'):
        lines.append("|" + format_line(f"     Conversion: {pattern.get('conversion', '')}") + "|")
    if pattern.get('cta_placement'):
        lines.append("|" + format_line(f"     CTA: {pattern.get('cta_placement', '')}") + "|")
    lines.append("|" + format_line("     Sections:") + "|")
    for i, section in enumerate(sections, 1):
        lines.append("|" + format_line(f"       {i}. {section}") + "|")
    lines.append("|" + " " * w + "|")

    # Style section
    lines.append("|" + format_line(f"  STYLE: {style.get('name', '')}") + "|")
    if style.get("keywords"):
        for line in wrap_text(f"Keywords: {style.get('keywords', '')}", "|     ", w):
            lines.append("|" + format_line(line[1:]) + "|")
    if style.get("best_for"):
        for line in wrap_text(f"Best For: {style.get('best_for', '')}", "|     ", w):
            lines.append("|" + format_line(line[1:]) + "|")
    if style.get("performance") or style.get("accessibility"):
        perf_a11y = f"Performance: {style.get('performance', '')} | Accessibility: {style.get('accessibility', '')}"
        lines.append("|" + format_line(f"     {perf_a11y}") + "|")
    lines.append("|" + " " * w + "|")

    # Colors section
    lines.append("|" + format_line("  COLORS:") + "|")
    lines.append("|" + format_line(f"     Primary:    {colors.get('primary', '')}") + "|")
    lines.append("|" + format_line(f"     Secondary:  {colors.get('secondary', '')}") + "|")
    lines.append("|" + format_line(f"     CTA:        {colors.get('cta', '')}") + "|")
    lines.append("|" + format_line(f"     Background: {colors.get('background', '')}") + "|")
    lines.append("|" + format_line(f"     Text:       {colors.get('text', '')}") + "|")
    if colors.get("notes"):
        for line in wrap_text(f"Notes: {colors.get('notes', '')}", "|     ", w):
            lines.append("|" + format_line(line[1:]) + "|")
    lines.append("|" + " " * w + "|")

    # Typography section
    lines.append("|" + format_line(f"  TYPOGRAPHY: {typography.get('heading', '')} / {typography.get('body', '')}") + "|")
    if typography.get("mood"):
        for line in wrap_text(f"Mood: {typography.get('mood', '')}", "|     ", w):
            lines.append("|" + format_line(line[1:]) + "|")
    if typography.get("best_for"):
        for line in wrap_text(f"Best For: {typography.get('best_for', '')}", "|     ", w):
            lines.append("|" + format_line(line[1:]) + "|")
    if typography.get("google_fonts_url"):
        lines.append("|" + format_line(f"     Google Fonts: {typography.get('google_fonts_url', '')}") + "|")
    if typography.get("css_import"):
        lines.append("|" + format_line(f"     CSS Import: {typography.get('css_import', '')[:70]}...") + "|")
    lines.append("|" + " " * w + "|")

    # Key Effects section
    if effects:
        lines.append("|" + format_line("  KEY EFFECTS:") + "|")
        for line in wrap_text(effects, "|     ", w):
            lines.append("|" + format_line(line[1:]) + "|")
        lines.append("|" + " " * w + "|")

    # Anti-patterns section
    if anti_patterns:
        lines.append("|" + format_line("  AVOID (Anti-patterns):") + "|")
        for line in wrap_text(anti_patterns, "|     ", w):
            lines.append("|" + format_line(line[1:]) + "|")
        lines.append("|" + " " * w + "|")

    # Pre-Delivery Checklist section
    lines.append("|" + format_line("  PRE-DELIVERY CHECKLIST:") + "|")
    checklist_items = [
    ...
    for item in checklist_items:
        lines.append("|" + format_line(f"     {item}") + "|")
    lines.append("|" + " " * w + "|")
...
def persist_design_system(design_system: dict, page: str = None, output_dir: str = None, page_query: str = None) -> dict:
    ...
    project_slug = re.sub(r'[^a-z0-9\-_]', '', project_name.lower().replace(' ', '-')) or 'default'

...
def derive_row(pt, pri, sec, acc, bg, notes=""):
    """Generate full 18-token color row from 4 base colors."""
    dark = is_dark(bg)
    ...
    border = blend(bg, "#FFFFFF", 0.08) if dark else blend("#FFFFFF", pri, 0.12)
    destr = "#DC2626"
    on_destr = "#FFFFFF"
    ring = pri
...
def rebuild_colors():
    ...
    for i, prod in enumerate(products, 1):
        ...
        if pt in color_map:
            ...
            if len(headers) != 1 + len(new_row):
                raise ValueError(f"Header count mismatch: {len(headers)} headers vs {1 + len(new_row)} values")
